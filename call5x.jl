...
@show interclass; @show lengthscales; @show usefix; @show suminter; @show gpnum; @show rmfl;
...
for lengthscale in lengthscales
	...
	for i = 1:repeats
		...
		open("../logs/log_gnw.csv", "a") do f
			...
			# write(f, "\n")
			write(f, "$(cnt+i-1)\t$(now())\t$numspecies\t$srcset\t$tspan\t$δt\t$σ\t$maxinter\t$interclass\t$lengthscale\t$usefix\t$suminter\t$gpnum\t$rmfl\t$aupr_aic\t$aupr_bic\t$auroc_aic\t$auroc_bic\t$thalia_aupr\t$thalia_auroc\t$ranks\t$bestmodels\t$truedges\t$othersum\t$edgeweights")
			write(f, "\n")
		end
	...
	open("../logs/logmean_gnw.csv", "a") do f
		# write(f, "id\ttime\tnumspecies\tsrcset\ttspan\tdt\tsigma\tmaxinter\tinterclass\tlengthscale\tusefix\tsuminter\tgpnum\trmfl\taupr_aic\taupr_bic\tauroc_aic\tauroc_bic\tthalia_aupr\tthalia_auroc\ttruedges\tothersum\tedgeweights")
		# write(f, "\n")
		write(f, "$cnt\t$(now())\t$numspecies\t$srcset\t$tspan\t$δt\t$σ\t$maxinter\t$interclass\t$lengthscale\t$usefix\t$suminter\t$gpnum\t$rmfl\t$avg_aupr_aic\t$avg_aupr_bic\t$avg_auroc_aic\t$avg_auroc_bic\t$avg_thalia_aupr\t$avg_thalia_auroc\t$avg_truedges\t$avg_othersum\t$avg_edgeweights")
		write(f, "\n")
	end
end
